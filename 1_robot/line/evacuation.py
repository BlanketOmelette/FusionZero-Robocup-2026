...
class EvacController:
    ...
    # ----------------------------
    def reset(self) -> None:
        self.state = EvacState.ENTER_DRIVE
    ...
    def step(self, robot_state, evac_frame, line_frame) -> None:
        ...
        if self.phase == MissionPhase.FIND_EXIT:
            self.exit(robot_state)
            listener.set_mode(1)                   # back to normal mode
            setattr(robot_state, "evac_exited", True)
            return
        ...
        if (
            self.entered_evac
            and self.phase != MissionPhase.FIND_EXIT
            and now >= self.boundary_cooldown_until
            and self.state not in (EvacState.GRAB, EvacState.DUMP, EvacState.HIT_RECOVER, EvacState.TILT_RECOVER)
        ):
            saw_silver = self.detect_entry_silver_line(line_frame)
            saw_black  = self.detect_exit_black_line(line_frame)

            if saw_silver or saw_black:
                self.boundary_cooldown_until = now + self.BOUNDARY_COOLDOWN_SECONDS
                self.target = None
    ...
    def exit(self, robot_state) -> bool:
        """
        Your exit method goes here (line follower, special behavior, etc).
        Return True when you have exited and want to stop evac mode.
        """
        # TODO: implement
        print("exiting evac...")
        motors.run(0, 0, 10)
